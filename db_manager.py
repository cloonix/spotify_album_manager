...
def fetch_items(conn):
    ...
    c.execute(
        "SELECT id, artist, item_name, release_date, release_year, item_url, type FROM items ORDER BY artist"
    )
    return c.fetchall()
...
def fetch_items_by_artist(conn, artist):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT * FROM items WHERE artist = ? ORDER BY release_date DESC", (artist,)
    )
    return c.fetchall()
